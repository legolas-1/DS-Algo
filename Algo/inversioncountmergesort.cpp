...
/* This algorithm is use to find number of
   inversion in an array, an inversion si where
   i < j and A[i] > A[j] where i and j are in
   indexes and A is an array. It uses merge sort 
   to find out this count. At line 72 it says add
   number of elements where L[i] will be greater in 
   R[]. keep on adding count like this will give
   total number of inversion count. Overall complexity
